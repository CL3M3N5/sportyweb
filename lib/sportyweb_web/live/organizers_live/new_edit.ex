defmodule SportywebWeb.EventOrganizerLive.NewEdit do
  ...
  def render(assigns) do
    ...
    <div>
      <.live_component
        module={SportywebWeb.EventOrganizerLive.FormComponent}
        id="contact-form"
        title={@page_title}
        event={@event}
  ...
  defp apply_action(socket, :new, %{"id" => event_id}) do
    ...
    socket
    |> assign(:page_title, "Organisator hinzufügen")
    |> assign(:event, event)
  ...
  defp apply_action( socket, :delete, %{"event_id" => event_id, "contact_id" => contact_id} ) do
    event_contact = Calendar.get_event_contact!(event_id, contact_id, "organizer")
    {:ok, _} = Calendar.delete_event_contact(event_contact)

    socket
    |> put_flash(:info, "Organistor erfolgreich gelöscht")
